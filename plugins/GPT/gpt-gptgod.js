...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if (!db.data.dbai.gptgod.name && !db.data.dbai.gptgod.profile)
    return m.reply(
  ...
  if ("gptgod" === command) {
    ...
    try {
      ...
      output
        ? m.reply(`*${db.data.dbai.gptgod.name}*\n\n${output}`)
...
async function chatAI(query, profile, model) {
  const options = {
    ...
    headers: {
      ...
      Authorization:
        "Bearer sk-7IAYvtiat58wTdD155QtWob01oCRe44yGLUOPFe1BxEShnOY",
    },
    body: JSON.stringify({
  ...
  try {
    const response = await fetch(
        "https://gptgod.site/api/v1/chat/completions",
        options,
      ),
