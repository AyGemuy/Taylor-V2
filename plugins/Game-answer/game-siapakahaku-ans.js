...
export async function before(m) {
  ...
  if (
    ...
    !m.text ||
    !(
      /🕹️ GAME - SIAPAKAHAKU[\s\S]*Balas pesan ini untuk menjawab/i.test(
        m.text || "",
      ) ||
      /🕹️ GAME - SIAPAKAHAKU[\s\S]*Balas pesan ini untuk menjawab/i.test(
        m.quoted?.text || "",
  ...
  if (
    ((db.data.game.siapakahaku = db.data.game.siapakahaku
      ...
      : {}),
    !(id in db.data.game.siapakahaku))
  )
    return await this.reply(m.chat, "Soal siapakahaku itu telah berakhir", m);
  if (m.quoted?.id === db.data.game.siapakahaku[id][0]?.id) {
    if (/^((me)?nyerah|surr?ender)$/i.test(m.text))
      return (
        clearTimeout(db.data.game.siapakahaku[id][3]),
    ...
    m.text.toLowerCase() === json.jawaban.toLowerCase().trim()
      ? ((db.data.users[m.sender].exp += db.data.game.siapakahaku[id][2]),
        await this.reply(
          m.chat,
          `✅ *Benar!*\n+${db.data.game.siapakahaku[id][2]} XP`,
          m,
          {
            contextInfo: {
              mentionedJid: [m.sender],
