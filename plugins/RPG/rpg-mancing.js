      ...
      switch (type) {
        case "emosi":
          ...
          if (new Date() - pengguna.lastfishing > 24e4) {
            ...
            setTimeout(async () => {
              await conn.reply(m.chat, mcng, null),
                psepick > 0 &&
                  ((pengguna.psepick += 1 * psepick),
                  await conn.reply(
                    m.chat,
              ...
              (pengguna.nila += 1 * nila),
              (pengguna.ikan += 1 * ikan),
              (pengguna.lele += 1 * lele),
              (pengguna.bawal += 1 * bawal),
              (pengguna.udang += 1 * udang),
              (pengguna.lastfishing += 1 * new Date()),
              (pengguna.paus += 1 * paus),
              (pengguna.kepiting += 1 * kepiting);
          } else m.reply(`You're already fishing, wait until ${timers}`);
          break;
        case "easy":
          let timersa = clockString(
            288e5 - (new Date() - pengguna.lastmancingeasy),
          );
          if (0 === pancingan || 0 === umpan)
            return await conn.reply(
              m.chat,
              "*[❗] kamu tidak punya kail pancingan dan umpan 100 🎣*",
              m,
            ...
            if (pengguna.pancingan > 1)
              if (pengguna.umpan > 99) {
                ...
                (pengguna.paus += rbrb1),
                  ...
                  (pengguna.lastmancingeasy = 1 * new Date());
              } else
                await conn.reply(
                  m.chat,
