...
const handler = async (m, { args, command }) => {
  try {
    const q = m.quoted || m;
    if ("application/zip" !== ((q.msg || q).mimetype || ""))
      throw 'Invalid media type. Only "application/zip" is allowed.';
    const buffer = await q?.download(),
      fileSizeInBytes = buffer.length;
    if (fileSizeInBytes / 1048576 > 3)
      throw "Input file size is too large. It must be below 3 MB.";
        ...
        obfuscatePromises.push(
          (async (zipEntry) => {
            ...
            try {
              ...
              zip.updateFile(
                zipEntry.entryName,
                Buffer.from(obfuscatedCode.getObfuscatedCode(), "utf8"),
              ),
                obfuscatedFiles.push(zipEntry.entryName);
            } catch (error) {
              console.error(
                `Failed to obfuscate ${zipEntry.entryName}: ${error.message}`,
              ),
    ...
    obfuscatedFiles.length > 0 &&
      (message += `*Obfuscated files: ${obfuscatedFiles.length}*\n`),
      errorFiles.length > 0 &&
    ...
    return (
      ...
      m.reply(`An error occurred while obfuscating file: ${err}`)
    );
  }
};
