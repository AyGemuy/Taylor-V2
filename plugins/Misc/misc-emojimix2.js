...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    ...
    if (!emojiA || !emojiB) {
      return m.reply(
        `*⛌ Masukkan emoji yang ingin kamu gabungkan.*\n\n- *Example:*\n- *${usedPrefix}${command}* 🐱+👻\n\n[ minimal 2 emoji ]`,
      );
    }
    m.react(wait);
    ...
    if (!anu || !anu.results || anu.results.length === 0) {
      return m.reply(
        "❌ *Tidak dapat menemukan stiker dengan emoji yang diberikan.*",
      );
    }
...
async function makerMix(a, b) {
  try {
    let response = await fetch(
      `https://api.andeer.top/API/emojimix.php?emoji1=${encodeURIComponent(a)}&emoji2=${encodeURIComponent(b)}`,
    );
