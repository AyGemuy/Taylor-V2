...
const handler = async (m, { conn }) => {
  ...
  const participantCounts = _.chain(messages)
    .values()
    .groupBy("key.participant")
    .mapValues("length")
    .value();
  const sortedData = _.chain(Object.entries(participantCounts))
    .sortBy(([, total]) => -total)
    .value();
  const totalM = _.sumBy(sortedData, ([, total]) => total);
  const totalPeople = sortedData.length;
  const pesan = _.chain(sortedData)
    .map(
      ([jid, total], index) =>
