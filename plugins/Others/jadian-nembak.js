...
const handler = async (
  ...
  if (isNaN(number))
    return await conn.reply(m.chat, "_*Nomor tidak valid.*_", m);
  if (number.length > 15)
    return await conn.reply(m.chat, "*_Format Tidak Valid.*_", m);
  try {
    ...
    if (void 0 === db.data.users[user])
      return m.reply(
        "_*Orang yang anda tag tidak terdaftar di dalam database.*_",
      );
    var pacar = db.data.users[user].pasangan,
      spac = db.data.users[m.sender].pasangan;
    if (
      "" != db.data.users[m.sender].pasangan &&
      db.data.users[db.data.users[m.sender].pasangan].pasangan === m.sender &&
      ...
      if (pacar) {
        if (m.sender === pacar && db.data.users[m.sender].pasangan === user)
          return await conn.reply(
            m.chat,
