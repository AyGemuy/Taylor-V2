const handler = async (m, { conn, command }) => {
  ...
  if ("lockcmd" === command)
    (sticker[hash].locked = !0),
      await conn.reply(m.chat, "Perintah stiker berhasil dikunci! 🔒", m);
  else {
    if ("unlockcmd" !== command)
      throw `Perintah tidak valid. Gunakan *${command}cmd* untuk mengunci atau membuka kunci perintah.`;
    (sticker[hash].locked = !1),
      await conn.reply(m.chat, "Perintah stiker berhasil dibuka kunci! 🔓", m);
  }
};
