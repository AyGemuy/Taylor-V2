...
const fetchQuranData = async (surahNumber) => {
    try {
      const response = await fetch(
        `https://quran-wudy.vercel.app/surah/${surahNumber}`,
      );
  ...
  handler = async (m, { conn }) => {
    db.data.dbbot.qurannData = db.data.dbbot.qurannData
      ? db.data.dbbot.qurannData
      : {};
    ...
    const formattedList = Object.values(ayahs)
      .map((v) => `*${v.number.inSurah}.* ${v.text.arab}`)
      .join("\n");
    let { key } = m.reply(
...
(handler.before = async (m, { conn }) => {
  if (
    ((db.data.dbbot.qurannData = db.data.dbbot.qurannData
      ? db.data.dbbot.qurannData
      : {}),
    m.isBaileys || !(m.chat in db.data.dbbot.qurannData))
  )
    return;
  const input = m.text.trim();
  if (!/^\d+$/.test(input)) return;
  const { list, key } = db.data.dbbot.qurannData[m.chat];
  ...
  if (isNaN(index) || index < 1 || index > list.length)
    ...
    await conn.sendMessage(
      ...
      clearTimeout(db.data.dbbot.qurannData[m.chat].timeout),
      delete db.data.dbbot.qurannData[m.chat];
  }
}),
  (handler.help = ["qurann"]),
