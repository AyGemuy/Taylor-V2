import { oploverz } from "../../lib/scraper/all/anime.js";
const handler = async (m, { text }) => {
  ...
  try {
    if (!query) {
      return m.reply(
        "ℹ️ Masukkan query untuk mencari konten di Oploverz.\nContoh penggunaan: oploverz Naruto",
      );
    }
    if (!index && !episode) {
...
- Download Links:`;
  ...
  ${quality} (${server}): ${detailResult.download[quality][server]}`;
    ...
    if (
      isNaN(episode) ||
      episode <= 0 ||
      episode > selectedResult.episode.length
    ) {
      return m.reply(
        `Episode tidak valid. Mohon berikan nomor antara 1 dan ${selectedResult.episode.length}.`,
      );
    }
    const detailResult = await oploverz.detail(
      selectedResult.episode[episode - 1].url,
    );
...
handler.help = [
  "oploverz <query>|<index>|<episode>",
  "oploverz <query>|<index>",
  "oploverz <query>",
];
