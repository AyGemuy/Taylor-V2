...
const handler = async (m, { conn, usedPrefix, text, args, command }) => {
  ...
  if (lister.includes(feature)) {
    ...
    "file" === feature &&
      (await conn.sendFile(m.chat, querys, "Boom nya kak!", "", m, !1, {
        asDocument: !0,
      }));
  }
};
...
async function getBookInfo(type, query) {
  const response = await fetch(
      `https://www.googleapis.com/books/v1/volumes?q=${query}&download=epub&key=AIzaSyAobueeKqov3n13IofqiDWs6b1XsdU8Dr8`,
    ),
