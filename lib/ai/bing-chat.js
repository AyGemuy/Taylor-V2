...
class BingChat {
  ...
  async createConversation() {
    try {
      const response = await fetch(
        ...
        json = await response.json();
      (this.json.arguments[0].conversationSignature =
  ...
  async sendMessage(msg) {
    try {
          ...
          (chat.onmessage = (event) => {
            ...
            if (
              ("{}" === message &&
                ...
                (this.json.arguments[0].isStartOfSession = !1)),
              2 === json.type)
            ) {
              const response =
                json.item.messages
                  .filter((e) => "Chat" === e.messageType)
                  .reverse()[0] ||
                json.item.messages
                  .filter(
                    (e) =>
                      "bot" === e.author &&
                      "TextBlock" === e.adaptiveCards[0]?.body[0]?.type,
                  )
                  .reverse()[0];
              chat.close(),
                resolve({
                  ...
                  card: (
                    json.item.messages
                      .filter((e) => "InternalSearchResult" === e.messageType)
                      .reverse()[0] || {}
                  ).groundingInfo,
                  spokenText: (
                    json.item.messages
                      .filter((e) => null != e.spokenText)
                      .reverse()[0] || {}
                  ).spokenText,
                });
            }
      ...
      throw (
        (console.log(chalk.red("Error in sendMessage method:", error)), error)
      );
    }
  }
}
